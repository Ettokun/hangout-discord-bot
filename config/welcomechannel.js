        ...
        (err, guild) => {
            ...
            if (swtich.toLowerCase() === "none") {
                const LevelNotif = new MessageEmbed()
                    .setColor("GREEN")
                    .setDescription(
                        `**Welcome Channel** Has Been Update To : ${swtich}\n\nType ${prefix}config welcomechannel [Channel Name], to add new Configuratuion`
                    );

                msg.channel.send(LevelNotif);
                guild.configGuild.Channel.Welcome_Channel = "";
            ...
            const LevelNotif = new MessageEmbed()
                .setColor("GREEN")
                .setDescription(
                    `**Welcome Channel** Has Been Update To : ${swtich}\n\nType ${prefix}config welcomechannel none, to remove ${swtich} from config`
                );
            ...
            if (guild.configGuild.Channel.Leave_Channel.length <= 0) {
